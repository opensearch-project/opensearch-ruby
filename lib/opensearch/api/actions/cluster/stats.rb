  ...
  module API
    module Cluster
      module Actions
        ...
        # @option args [String] :timeout Period to wait for each node to respond. If a node does not respond before its timeout expires, the response does not include its stats. However, timed out nodes are included in the response's `_nodes.failed` property. Defaults to no timeout.
        ...
        # @option args [Enumerable<String>, String] :node_id Comma-separated list of node filters used to limit returned information. Defaults to all nodes in the cluster.
        def stats(args = {})
          ...
          method  = 'GET'
          url     = ['_cluster/stats', _metric, _index_metric, 'nodes', _node_id].filter(&:present?).join('/').squeeze('/')
