  ...
  module API
    module Indices
      module Actions
        # Closes an index.
        #
        # @option args [Enumerable<String>, String] :index *Required* Comma-separated list or wildcard expression of index names used to limit the request.
        ...
        def close(args = {})
          ...
          method  = 'POST'
          url     = [_index, '_close'].filter(&:present?).join('/').squeeze('/')
