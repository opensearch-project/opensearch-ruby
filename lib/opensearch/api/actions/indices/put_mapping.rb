  ...
  module API
    module Indices
      module Actions
        ...
        #
        # @option args [Enumerable<String>, String] :index *Required* A comma-separated list of index names the mapping should be added to (supports wildcards); use `_all` or omit to add the mapping on all indexes.
        ...
        # @option args [Boolean] :write_index_only If `true`, the mappings are applied only to the current write index for the target.
        # @option args [Hash] :body *Required* The mapping definition
        def put_mapping(args = {})
          ...
          method  = 'POST'
          url     = [_index, '_mapping'].filter(&:present?).join('/').squeeze('/')

          Utils.validate_query_params args, PUT_MAPPING_QUERY_PARAMS
          transport.perform_request(method, url, args, body, headers).body
        end

        PUT_MAPPING_QUERY_PARAMS = Set.new(%w[
          allow_no_indices
