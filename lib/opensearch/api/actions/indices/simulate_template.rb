  ...
  module API
    module Indices
      module Actions
        ...
        #
        # @option args [String] :cause User defined reason for dry-run creating the new template for simulation purposes.
        ...
        def simulate_template(args = {})
          ...
          method  = 'POST'
          url     = ['_index_template/_simulate', _name].filter(&:present?).join('/').squeeze('/')
