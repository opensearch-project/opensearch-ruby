  ...
  module API
    module Indices
      module Actions
        ...
        # @option args [Boolean] :forbid_closed_indices If `true`, statistics are not collected from closed indexes.
        ...
        def stats(args = {})
          ...
          method  = 'GET'
          url     = [_index, '_stats', _metric].filter(&:present?).join('/').squeeze('/')
