  ...
  module API
    module Root
      module Actions
        ...
        #
        # @option args [Boolean] :ccs_minimize_roundtrips If `true`, network round-trips between the coordinating node and remote clusters are minimized for cross-cluster search requests.
        # @option args [Integer] :max_concurrent_searches Maximum number of concurrent searches the multi search API can execute.
        # @option args [Integer] :max_concurrent_shard_requests Maximum number of concurrent shard requests that each sub-search request executes per node.
        ...
        def msearch(args = {})
          ...
          url     = [_index, '_msearch'].filter(&:present?).join('/').squeeze('/')
