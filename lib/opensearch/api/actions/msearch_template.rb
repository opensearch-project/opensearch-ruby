  ...
  module API
    module Root
      module Actions
        ...
        #
        # @option args [Boolean] :ccs_minimize_roundtrips If `true`, network round-trips are minimized for cross-cluster search requests.
        ...
        def msearch_template(args = {})
          ...
          url     = [_index, '_msearch/template'].filter(&:present?).join('/').squeeze('/')
