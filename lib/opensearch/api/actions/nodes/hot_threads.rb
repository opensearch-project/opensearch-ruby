  ...
  module API
    module Nodes
      module Actions
        ...
        # @option args [Boolean] :ignore_idle_threads Don't show threads that are in known-idle places, such as waiting on a socket select or pulling from an empty task queue.
        # @option args [String] :interval The interval for the second sampling of threads.
        # @option args [Integer] :snapshots Number of samples of thread stack trace.
        # @option args [Integer] :threads Specify the number of threads to provide information for.
        ...
        def hot_threads(args = {})
          ...
          method  = 'GET'
          url     = ['_nodes', _node_id, 'hot_threads'].filter(&:present?).join('/').squeeze('/')
