...
describe 'client.indices#exists_alias' do
  ...
  it 'performs the request' do
    expect(client_double.indices.exists_alias(name: 'foo')).to be(true)
  end
  ...
  context 'when multiple indices are specified' do
    ...
    it 'performs the request' do
      expect(client_double.indices.exists_alias(index: %w[foo bar], name: 'bam')).to be(true)
    end
  end
  ...
  context 'when the path needs to be URL-escaped' do
    ...
    it 'performs the request' do
      expect(client_double.indices.exists_alias(index: 'foo^bar', name: 'bar/bam')).to be(true)
    end
  end
  ...
  context 'when 404 response is received' do
    ...
    it 'returns false' do
      expect(client_double.indices.exists_alias(name: 'foo')).to be(false)
    end
  end
  ...
  context 'when a \'not found\' exception is raised' do
    ...
    it 'returns false' do
      expect(client.indices.exists_alias(name: 'foo')).to be(false)
    end
  end
