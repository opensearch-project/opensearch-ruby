...
describe 'Rest API YAML tests' do
  if REST_API_YAML_FILES.empty?
    ...
    logger.info 'Use rake rake opensearch:download_artifacts in the root directory of the project to download the test artifacts.'
    exit 1
  end
  ...
  REST_API_YAML_FILES.each do |file|
    begin
      test_file = OpenSearch::RestAPIYAMLTests::TestFile.new(file, ADMIN_CLIENT, REST_API_YAML_SKIP_FEATURES)
      ...
      test_file.tests.each do |test|
        context "#{test.description}" do
          if test.skip_test?(ADMIN_CLIENT)
            ...
            before(:all) do
              OpenSearch::RestAPIYAMLTests::TestFile.wipe_cluster(ADMIN_CLIENT)
              test_file.setup
            end
