...
module Opensearch
  module Extensions
    module Test
      ...
      #          assuming `opensearch` is on $PATH.
      #
      ...
      #      system 'tar -xvf opensearch-5.1.1.tar.gz'
      #
      #      require 'opensearch/extensions/test/cluster'
      #      Opensearch::Extensions::Test::Cluster.start command: 'opensearch-5.1.1/bin/opensearch'
      #
      ...
      module Cluster
        ...
        class Cluster
          ...
          # @option arguments [String]  :command      Elasticsearch command (default: `opensearch`)
          ...
          # @option arguments [Integer] :timeout_version Timeout when waiting for `opensearch --version` (default: 15)
          ...
          #      Opensearch::Extensions::Test::Cluster::Cluster.new(
          #        command: "/usr/local/Cellar/opensearch/1.0.0.Beta2/bin/opensearch"
          #      ).start
          #
          ...
          # if not available, it parses the version number from the `lib/opensearch-X.Y.Z.jar` file,
          # if that is not available, uses `opensearch --version` or `opensearch -v`
          #
          # @api private
          #
          # @return String
          ...
          def __determine_version
            ...
            version = if arguments[:version]
              arguments[:version]
            elsif File.exist?(path_to_lib) && !(jar = Dir.entries(path_to_lib).select { |f| f =~ /^opensearch\-\d/ }.first).nil?
              __log "Determining version from [#{jar}]" if ENV['DEBUG']
              if m = jar.match(/opensearch\-(\d+\.\d+\.\d+).*/)
                m[1]
              else
              ...
              if m = output.match(/Version: (\d\.\d.\d).*,/)
                ...
                raise RuntimeError, "Cannot determine Elasticsearch version from opensearch --version output [#{output}]"
              end
            end
