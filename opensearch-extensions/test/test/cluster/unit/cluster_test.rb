...
class Opensearch::Extensions::TestClusterTest < Opensearch::Test::UnitTestCase
  ...
  context "The Test::Cluster" do
    ...
    context "class" do
      ...
      should "have a default cluster name" do
        ...
        assert_equal 'opensearch-test-foobar', Cluster::Cluster.new.__default_cluster_name
      end
      ...
      context "when determining a version" do
        setup do
          @subject = Opensearch::Extensions::Test::Cluster::Cluster.new command: '/foo/bar/bin/opensearch'
        end

        should "return version from lib/opensearch.X.Y.Z.jar" do
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['foo.jar', 'opensearch-foo-1.0.0.jar', 'opensearch-2.3.0.jar', 'opensearch-bar-9.9.9.jar'])

          assert_equal '2.0', @subject.__determine_version
        end

        should "return version from `opensearch --version`" do
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(false)
          File.expects(:exist?).with('/foo/bar/bin/opensearch').returns(true)
        ...
        should "return version from arguments" do
          cluster = Opensearch::Extensions::Test::Cluster::Cluster.new command: '/foo/bar/bin/opensearch', version: '5.2'
        ...
        should "raise an exception when the version cannot be parsed from .jar" do
          ...
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['opensearch-100.jar'])
        ...
        should "raise an exception when the version cannot be converted to short version" do
          ...
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['opensearch-3.2.1.jar'])
