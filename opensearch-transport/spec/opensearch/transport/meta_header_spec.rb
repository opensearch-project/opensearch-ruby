...
describe Opensearch::Transport::Client do
  context 'meta-header' do
    ...
    let(:meta_header) do
      if jruby?
        "es=#{meta_version},rb=#{RUBY_VERSION},t=#{Opensearch::Transport::VERSION},jv=#{ENV_JAVA['java.version']},jr=#{JRUBY_VERSION},fd=#{Faraday::VERSION},#{adapter_code}"
      else
        "es=#{meta_version},rb=#{RUBY_VERSION},t=#{Opensearch::Transport::VERSION},fd=#{Faraday::VERSION},#{adapter_code}"
      end
    end
    ...
    # Opensearch version in the meta header string dynamic
    def meta_version
      client.send(:client_meta_version, Opensearch::VERSION)
    end
    ...
    context 'adapters' do
      let(:meta_header) do
        if jruby?
          "es=#{meta_version},rb=#{RUBY_VERSION},t=#{Opensearch::Transport::VERSION},jv=#{ENV_JAVA['java.version']},jr=#{JRUBY_VERSION},fd=#{Faraday::VERSION}"
        else
          "es=#{meta_version},rb=#{RUBY_VERSION},t=#{Opensearch::Transport::VERSION},fd=#{Faraday::VERSION}"
        end
      end
    ...
    if defined?(JRUBY_VERSION)
      context 'when using manticore' do
        let(:client) do
          described_class.new(transport_class: Opensearch::Transport::Transport::HTTP::Manticore).tap do |client|
      ...
      context 'when using curb' do
        let(:client) do
          described_class.new(transport_class: Opensearch::Transport::Transport::HTTP::Curb).tap do |client|
            client.instance_variable_set('@verified', true)
          end
        end
    ...
    context 'when using custom transport implementation' do
      ...
      let(:client) { Opensearch::Transport::Client.new(transport_class: transport_class) }
      let(:subject) { client.instance_variable_get('@arguments')[:transport_options][:headers] }
      let(:meta_header) do
        if jruby?
          "es=#{meta_version},rb=#{RUBY_VERSION},t=#{Opensearch::Transport::VERSION},jv=#{ENV_JAVA['java.version']},jr=#{JRUBY_VERSION}"
        else
          "es=#{meta_version},rb=#{RUBY_VERSION},t=#{Opensearch::Transport::VERSION}"
        end
      end
    ...
    context 'when using a different service version' do
      before do
        stub_const('Elastic::Opensearch_SERVICE_VERSION', [:ent, '8.0.0'])
      end
